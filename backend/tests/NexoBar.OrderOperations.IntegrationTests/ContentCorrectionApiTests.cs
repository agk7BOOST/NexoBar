...
public sealed class ContentCorrectionApiTests(OrderOperationsApiFixture fixture)
{
    [Theory]
    ...
    public async Task Malformed_body_and_client_actor_are_rejected(string body)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, ContentCorrectionTestSupport.Path(target))
        ...
        await ContentCorrectionTestSupport.ProblemAsync(response, HttpStatusCode.BadRequest, "request_invalid", token);
    ...
    public async Task Structural_guards_return_problem_details(string kind)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, ContentCorrectionTestSupport.Path(target)) { Content = JsonContent.Create(new CorrectContentRequest(1)) };
        ...
        await ContentCorrectionTestSupport.ProblemAsync(response, HttpStatusCode.BadRequest, code, token);
    }
    ...
    public async Task Exact_target_must_exist_in_order(string kind)
    {
        ...
        await ContentCorrectionTestSupport.ProblemAsync(response, HttpStatusCode.NotFound, "content_not_found", token);
        await ContentCorrectionTestSupport.CountsAsync(fixture, 0, token);
    }
    ...
    public async Task Frozen_and_closed_orders_reject_correction(bool close)
    {
        ...
        await fixture.ResetAsync(token);
        var target = await DeliveryQuantityTestSupport.CreateDirectAsync(fixture, 3, token);
        await ContentCorrectionTestSupport.DeliverAsync(fixture, target, 3, token);
    ...
    public async Task Failure_rolls_back_state_history_and_command(string suffix, bool prepared)
    {
        ...
        sql.CommandText = $"""
            ...
            CREATE TRIGGER fail_correction BEFORE INSERT ON order_operations.content_correction_{suffix}
        ...
        try
        {
            ...
            Assert.Equal(0, (await db.ContentQuantityStates.SingleAsync(token)).RemovedByCorrectionQuantity);
        ...
        {
            sql.CommandText = $"DROP TRIGGER fail_correction ON order_operations.content_correction_{suffix}; DROP FUNCTION order_operations.fail_correction();";
            await sql.ExecuteNonQueryAsync(token);
        }
        using var retry = await ContentCorrectionTestSupport.PostAsync(fixture.OrderOperationsClient, target, key, 1, token);
        await ContentCorrectionTestSupport.SuccessAsync(retry, token);
    }
}

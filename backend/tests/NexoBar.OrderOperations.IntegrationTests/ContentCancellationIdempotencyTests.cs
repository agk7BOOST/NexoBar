...
public sealed class ContentCancellationIdempotencyTests(OrderOperationsApiFixture fixture)
{
    [Fact]
    public async Task Replay_preserves_original_result_after_other_delivery_freeze_and_revocation()
    {
        ...
        var original = await ContentCancellationTestSupport.SuccessAsync(response, token);
        await ContentCancellationTestSupport.DeliverAsync(fixture, target, 2, token);
        ...
        using var replay = await ContentCancellationTestSupport.PostAsync(fixture.OrderOperationsClient, target, key, 1, token).WaitAsync(TimeSpan.FromSeconds(10), token);
        Assert.Equal(original, await ContentCancellationTestSupport.SuccessAsync(replay, token));
        using var newIntent = await ContentCancellationTestSupport.PostAsync(fixture.OrderOperationsClient, target, Guid.NewGuid(), 1, token);
        await ContentCancellationTestSupport.ProblemAsync(newIntent, HttpStatusCode.Forbidden, "forbidden", token);
    ...
    public async Task Fingerprint_rejects_incompatible_actor_and_exact_intent(string change)
    {
        ...
        await ContentCancellationTestSupport.SuccessAsync(first, token);
        ...
        using var response = await ContentCancellationTestSupport.PostAsync(client, target, key, change == "quantity" ? 2 : 1, token);
        await ContentCancellationTestSupport.ProblemAsync(response, HttpStatusCode.Conflict, "idempotency_key_conflict", token);
        await ContentCancellationTestSupport.CountsAsync(fixture, 1, token);
    }
    ...
    public async Task Authentication_and_current_authority_are_required(string kind)
    {
        ...
        using var response = await ContentCancellationTestSupport.PostAsync(client, target, kind == "no-responsibility" ? Guid.NewGuid() : key, 1, token);

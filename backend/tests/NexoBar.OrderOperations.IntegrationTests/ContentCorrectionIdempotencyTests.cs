...
public sealed class ContentCorrectionIdempotencyTests(OrderOperationsApiFixture fixture)
{
    [Fact]
    public async Task Replay_preserves_original_result_after_other_delivery_freeze_and_revocation()
    {
        ...
        var original = await ContentCorrectionTestSupport.SuccessAsync(response, token);
        await ContentCorrectionTestSupport.DeliverAsync(fixture, target, 2, token);
        ...
        using var replay = await ContentCorrectionTestSupport.PostAsync(fixture.OrderOperationsClient, target, key, 1, token).WaitAsync(TimeSpan.FromSeconds(10), token);
        Assert.Equal(original, await ContentCorrectionTestSupport.SuccessAsync(replay, token));
        using var newIntent = await ContentCorrectionTestSupport.PostAsync(fixture.OrderOperationsClient, target, Guid.NewGuid(), 1, token);
        await ContentCorrectionTestSupport.ProblemAsync(newIntent, HttpStatusCode.Forbidden, "forbidden", token);
    ...
    public async Task Fingerprint_rejects_incompatible_actor_and_exact_intent(string change)
    {
        ...
        await ContentCorrectionTestSupport.SuccessAsync(first, token);
        ...
        using var response = await ContentCorrectionTestSupport.PostAsync(client, target, key, change == "quantity" ? 2 : 1, token);
        await ContentCorrectionTestSupport.ProblemAsync(response, HttpStatusCode.Conflict, "idempotency_key_conflict", token);
        await ContentCorrectionTestSupport.CountsAsync(fixture, 1, token);
    }
    ...
    public async Task Authentication_and_current_authority_are_required(string kind)
    {
        ...
        using var response = await ContentCorrectionTestSupport.PostAsync(client, target, kind == "no-responsibility" ? Guid.NewGuid() : key, 1, token);

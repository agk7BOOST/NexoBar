...
public sealed class ContentCancellationSemanticsTests(OrderOperationsApiFixture fixture)
{
    [Theory]
    [InlineData(false, 1)]
    [InlineData(false, 3)]
    [InlineData(true, 1)]
    [InlineData(true, 3)]
    public async Task Exact_cancellation_preserves_confirmation_and_delivery(bool prepared, int quantity)
    {
        ...
        using var response = await ContentCancellationTestSupport.PostAsync(fixture.OrderOperationsClient, target, Guid.NewGuid(), quantity, token);
        var result = await ContentCancellationTestSupport.SuccessAsync(response, token);
        Assert.Equal(3, result.ConfirmedQuantity);
        Assert.Equal(0, result.PreviousCancelledQuantity);
        Assert.Equal(quantity, result.ResultingCancelledQuantity);
        ...
        Assert.Equal(quantity, item.GetProperty("cancelledQuantity").GetInt32());
        ...
        var history = await db.ContentCancellationHistory.SingleAsync(token);
        Assert.Equal("ContentQuantityCancelled", history.EventKind);
        ...
        Assert.Equal(result.PreviousCancelledQuantity, history.PreviousCancelledQuantity);
        Assert.Equal(result.ResultingCancelledQuantity, history.ResultingCancelledQuantity);
    ...
    public async Task Only_exact_eligible_quantity_is_removed(bool prepared, int quantity, int status)
    {
        ...
        await ContentCancellationTestSupport.CountsAsync(fixture, status == 200 ? 1 : 0, token);
    }
    ...
    public async Task Missing_state_is_inconsistency_without_repair(string table)
    {
        ...
        await ContentCancellationTestSupport.ProblemAsync(response, HttpStatusCode.InternalServerError, "state_inconsistent", token);

...
La elegibilidad de Liquidation exige ausencia de PendingComposition, cumplimiento resuelto y Estado consistente, sin Liquidation previa. El read de Order expone `functionalAmount`, `isLiquidationEligible` y `liquidationBlockers`: `pending_composition`, `unresolved_fulfillment`, `state_inconsistent` y `already_liquidated`. Actualmente cumplimiento resuelto exige `DeliveredQuantity = F` por Content, con Estado consistente según [Q/R/F de S7-I2](confirmation.md#q-r-y-f-s7-i2); no anticipa un comando de Content Correction ni cantidades de Cancellation.

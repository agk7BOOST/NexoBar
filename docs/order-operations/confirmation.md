...
- CAN-01 aprueba el descarte atómico de PendingComposition dentro de Complete Order Cancellation, sin Confirmación, Content nuevo ni Discard público previo. CAN-03 impide nuevas Incorporations o Start de PendingComposition después de esa terminación. Estas decisiones, aún no declaradas implementadas, se registran en [S7-CAN-D](ending.md#complete-order-cancellation--decisiones-aprobadas-s7-can-d).
...
- OperationalIntervention INT-01..INT-07 está implementada verticalmente en S7-I6D. Complete Order Cancellation tiene decisiones aprobadas en [S7-CAN-D](ending.md#complete-order-cancellation--decisiones-aprobadas-s7-can-d), con implementación pendiente. La deuda restante se conserva en [pendientes](pending.md).
...
### Complete Order Cancellation — cantidades aprobadas S7-CAN-D

Una sola decisión explícita Order-level cancela atómicamente toda la obligación vigente. Requiere D efectivo igual a cero en todos los Contents; no corrige Confirmation ni niega preparación o Delivery históricas. No es un batch de comandos públicos independientes.
...
Cada reducción de T reduce F en la misma cantidad. Al finalizar, por Content se preservan Q y R, `C_final = C_anterior + F_anterior = Q - R`, `F=0` y `D=0`. En cada Prepared Content queda `P=I=Y=T=0`. C es la única deducción por cancelación; no se agregan contadores de cancelación por etapa. Se conservan Content y Work, y la [Historia semántica](contracts-and-history.md#complete-order-cancellation--historia-e-idempotencia-aprobadas-s7-can-d) explica la procedencia de las cantidades.

CAN-04 permite todos F=0 antes de la decisión sin fabricar hechos de cantidad cero. Todos F=0 no son por sí solos un marcador de Complete Cancellation: la [terminación CAN-03](ending.md#complete-order-cancellation--decisiones-aprobadas-s7-can-d) tiene Estado propio y no pasa por Liquidation/Closure. Estas son decisiones aprobadas, no una declaración de implementación.

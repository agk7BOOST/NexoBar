...
- Cada Content tiene exactamente un `ContentQuantityState`, con la misma identidad `(IncorporationId, ContentOrdinal)`, que conserva `RemovedByCorrectionQuantity = R` y la cantidad cancelada `C`. La obligación vigente es `F = Q - R - C`. INT-04 aprueba que `CancelledQuantity` C incluya Cancellation ordinaria desde Pending y Cancellation mediante OperationalIntervention; no se agrega otra deducción a F. Las transiciones de intervención están [aprobadas, pendientes de implementación](preparation.md#operationalintervention--decisiones-aprobadas-s7-int-d).
...
- La implementación de OperationalIntervention sigue pendiente bajo INT-01..07 aprobadas. Cancellation completa de Order, semántica física de desperdicio/descarte/recuperación, Applied Price Correction, reparación post-Liquidation y SSE permanecen en [pendientes](pending.md).
- Un `PreparationWork` nace con `TotalQuantity > 0`; Cancellation ordinaria puede reducirlo a cero junto con `PendingQuantity` cuando `F` llega a cero. Las transiciones para trabajo real `InPreparation` o `Ready` se definen en INT-01/02, con implementación pendiente.

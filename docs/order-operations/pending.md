...
Slice 8 continúa abierto. Los verticales `preparation.destination.changed` y `order.changed` para el Order explícitamente activo están implementados; este último no es feed general de Orders. SSE de Catalog, OperationalConfiguration y feed general de Identity/capabilities continúan diferidos; Inventory es la próxima frontera a evaluar antes del cierre de Slice 8. También permanecen diferidos listas/búsqueda global de Orders, Historia y fan-out multi-instancia. Las decisiones y la limitación de instancia activa están en [SSE y frescura multiusuario](../architecture/sse-and-freshness.md); este alcance no incluye reparación post-Liquidation.

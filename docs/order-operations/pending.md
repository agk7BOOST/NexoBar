...
Slice 8 continúa abierto. El transporte y el vertical `preparation.destination.changed` están implementados. El vertical aprobado `order.changed` queda limitado al Order explícitamente activo para operación y permanece pendiente de implementación; no es un feed general de Orders. Su siguiente frontera es **S8-I4A0 — Active Order read authorization retrofit**: alinear los GETs/reads operacionales con AD-SEC-05 antes de retomar S8-I4A. SSE de Catalog, Inventory y OperationalConfiguration continúan diferidos. Sus decisiones y la limitación de instancia activa están en [SSE y frescura multiusuario](../architecture/sse-and-freshness.md); este alcance no incluye reparación post-Liquidation.

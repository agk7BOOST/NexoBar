...
- `DeliveredQuantity` forma parte del read de Preparation para que el cliente calcule el límite de “Corregir listo” sin depender de autorización sobre Delivery. Las correcciones no se ofrecen para Orders Frozen/Closed.
- Cuando `ReadyQuantity == TotalQuantity`, el panel muestra “Todo listo”. No usa lenguaje de Delivery.
- Cada Work admite como máximo una mutación frontend activa, en fase mínima `submitting` o `uncertain`; los demás Work siguen operables. Una intención nueva obtiene `crypto.randomUUID()` y congela kind, endpoint, `WorkId`, quantity, body, key y antiforgery token.
